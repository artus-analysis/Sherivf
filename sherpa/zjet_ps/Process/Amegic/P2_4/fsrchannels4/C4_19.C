...
namespace PHASIC {
  class C4_19 : public Single_Channel {
    Info_Key m_kI_2_3,m_kI_4_23,m_kI_5_234,m_kZS_0;
    Vegas* p_vegas;
  public:
    C4_19(int,int,Flavour*,Integration_Info * const);
    ~C4_19();
...
extern "C" Single_Channel * Getter_C4_19(int nin,int nout,Flavour* fl,Integration_Info * const info) {
  return new C4_19(nin,nout,fl,info);
}

void C4_19::GeneratePoint(Vec4D * p,Cut_Data * cuts,double * _ran)
{
...
void C4_19::GenerateWeight(Vec4D* p,Cut_Data * cuts)
{
  double wt = 1.;
  Vec4D p2345=p[0]+p[1];
  double s2345_max = p2345.Abs2();
...
C4_19::C4_19(int nin,int nout,Flavour* fl,Integration_Info * const info)
       : Single_Channel(nin,nout,fl)
{
  name = std::string("C4_19");
  rannum = 8;
...
void C4_19::ISRInfo(int & type,double & mass,double & width)
{
  type  = 2;
  mass  = 0;
  width = 0.;
...
void C4_19::AddPoint(double Value)
{
  Single_Channel::AddPoint(Value);
  p_vegas->AddPoint(Value,rans);
}
std::string C4_19::ChID()
{
  return std::string("CG2$I_2_3$I_4_23$I_5_234$MTH_23$MTH_234$ZS_0$");
}
