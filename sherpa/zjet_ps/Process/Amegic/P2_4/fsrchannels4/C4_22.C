...
namespace PHASIC {
  class C4_22 : public Single_Channel {
    Info_Key m_kI_2_3,m_kI_4_235,m_kI_5_23,m_kZS_205;
    Vegas* p_vegas;
  public:
    C4_22(int,int,Flavour*,Integration_Info * const);
    ~C4_22();
...
extern "C" Single_Channel * Getter_C4_22(int nin,int nout,Flavour* fl,Integration_Info * const info) {
  return new C4_22(nin,nout,fl,info);
}

void C4_22::GeneratePoint(Vec4D * p,Cut_Data * cuts,double * _ran)
{
  ...
  CE.Isotropic2Momenta(p2345,s4,s235,p[4],p235,ran[2],ran[3]);
  CE.Isotropic2Momenta(p235,s5,s23,p[5],p23,ran[4],ran[5]);
  CE.Isotropic2Momenta(p23,s2,s3,p[2],p[3],ran[6],ran[7]);
}

void C4_22::GenerateWeight(Vec4D* p,Cut_Data * cuts)
{
  double wt = 1.;
  Vec4D p2345=p[0]+p[1];
  double s2345_max = p2345.Abs2();
  ...
  if (m_kI_4_235.Weight()==ATOOLS::UNDEFINED_WEIGHT)
    m_kI_4_235<<CE.Isotropic2Weight(p[4],p235,m_kI_4_235[0],m_kI_4_235[1]);
  wt *= m_kI_4_235.Weight();

  rans[2]= m_kI_4_235[0];
  rans[3]= m_kI_4_235[1];
  if (m_kI_5_23.Weight()==ATOOLS::UNDEFINED_WEIGHT)
    m_kI_5_23<<CE.Isotropic2Weight(p[5],p23,m_kI_5_23[0],m_kI_5_23[1]);
  wt *= m_kI_5_23.Weight();
...
C4_22::C4_22(int nin,int nout,Flavour* fl,Integration_Info * const info)
       : Single_Channel(nin,nout,fl)
{
  name = std::string("C4_22");
  rannum = 8;
  rans  = new double[rannum];
  m_kI_2_3.Assign(std::string("I_2_3"),2,0,info);
  m_kI_4_235.Assign(std::string("I_4_235"),2,0,info);
  m_kI_5_23.Assign(std::string("I_5_23"),2,0,info);
...
void C4_22::ISRInfo(int & type,double & mass,double & width)
{
  type  = 2;
  mass  = 205.1721;
  width = 0.;
...
void C4_22::AddPoint(double Value)
{
  Single_Channel::AddPoint(Value);
  p_vegas->AddPoint(Value,rans);
}
std::string C4_22::ChID()
{
  return std::string("CG2$I_2_3$I_4_235$I_5_23$MP23_23$MTH_235$ZS_205$");
}
