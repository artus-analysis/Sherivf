...
namespace PHASIC {
  class C4_21 : public Single_Channel {
    Info_Key m_kI_2_3,m_kI_4_235,m_kI_5_23,m_kZS_0;
    Vegas* p_vegas;
  public:
    C4_21(int,int,Flavour*,Integration_Info * const);
    ~C4_21();
...
extern "C" Single_Channel * Getter_C4_21(int nin,int nout,Flavour* fl,Integration_Info * const info) {
  return new C4_21(nin,nout,fl,info);
}

void C4_21::GeneratePoint(Vec4D * p,Cut_Data * cuts,double * _ran)
{
...
void C4_21::GenerateWeight(Vec4D* p,Cut_Data * cuts)
{
  double wt = 1.;
  Vec4D p2345=p[0]+p[1];
  double s2345_max = p2345.Abs2();
...
C4_21::C4_21(int nin,int nout,Flavour* fl,Integration_Info * const info)
       : Single_Channel(nin,nout,fl)
{
  name = std::string("C4_21");
  rannum = 8;
...
void C4_21::ISRInfo(int & type,double & mass,double & width)
{
  type  = 2;
  mass  = 0;
  width = 0.;
...
void C4_21::AddPoint(double Value)
{
  Single_Channel::AddPoint(Value);
  p_vegas->AddPoint(Value,rans);
}
std::string C4_21::ChID()
{
  return std::string("CG2$I_2_3$I_4_235$I_5_23$MTH_23$MTH_235$ZS_0$");
}
