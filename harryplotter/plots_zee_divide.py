...
def divided_ptspectrum(args=None):
	...
	cut_bins = 1  # n outer bins to be cut away

	nbins = len(common.ybin_plotlabels[:-cut_bins])

	for quantity in common.data_quantities:
		filenames = []
		x_expressions = []
		nicks = []
		for index, ybin in enumerate(common.ybin_labels[:-cut_bins]):
		...
		d = {
			...
			'energies': [8],
			'labels': common.ybin_plotlabels[:-cut_bins]+[None]*nmc,
