...
def plot_pdf_unc_comparison(args=None, additional_dictionary=None, scenario='hera2_abszy'):
	...
	for only_hera in [True, False]:
		nicks = ["hera", "heracms"][:(1 if only_hera else 99)]
		scenarios = ['hera2', scenario][:(1 if only_hera else 99)]
		labels = [common.hera_title, common.hera_cms_title_short][:(1 if only_hera else 99)]
			...
			for only_exp in [True, False]:
				for flavour in common.pdf_unc_flavours:
					for _print in [True, False]:
						d = {
							#input
							'files': [os.path.join("results/"+scenario+"/pdf_" + pdfq + ".root") for scenario in scenarios],
							'folders': [''],
							...
							'colors': ([histo_colors['blue'], histo_colors['red']][:(1 if only_hera else 99)])*2,
							'subplot_legend': 'upper center',
							'texts': ["\n".join([flavour.replace("_", " "), text])],
							'x_lims': [1e-4, 0.9],
							'title': ("Only experimental uncertainties" if only_exp else ""),
							# output
							'filename': '_'.join([flavour, ("exp" if only_exp else "expmodelpar"), pdfq])+('_print' if _print else '')+('_onlyhera' if only_hera else ''),
						}
						if False:  # add line with diff of uncertainties
							...
							d['y_errors'] += [' ']
							d['colors'] += ['black']
						if float(".".join(pdfq.split('_')[:2])) < 2 and flavour in y_lims:
							d['y_lims'] = y_lims[flavour]
...
def plot_pdf_unc_comparison_all(args=None, additional_dictionary=None, scenario='hera2_zpt_bins'):
	...
	texts = [r'gluon $(\\!\\times {})$'.format(scaledict['gluon']), r'd valence', 'u valence', r'sea quarks $(\\!\\times {})$'.format(scaledict['sea_quarks']), 'quark', 'quark']
	...
	for only_exp in [True, False]:
		...
		for _print in [True, False]:
			d = {
				...
				'texts': [text],#+texts,
				'texts_x':[0.03],#, 0.45, 0.71, 0.73, 0.02]+[0.71, 0.73],
				'texts_y':[0.97],#, 0.81, 0.48, 0.8, 0.78]+[0.38, 0.7],
				'x_lims': [1e-4, 0.9],
				'y_lims': [0, 1],
				#'hatches': [None, '////']*4,
				'title': ("Only experimental uncertainties" if only_exp else ""),
